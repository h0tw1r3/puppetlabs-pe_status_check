...
Facter.add(:puppet_status_check, type: :aggregate) do
  ...
  chunk(:S0041) do
    next unless ['compiler'].include?(PuppetStatusCheck.config('role'))

    # Is pcp broker connected to another broker
    result = Facter::Core::Execution.execute("ss -onp state established '( dport = :8143 )' ", { timeout: PuppetStatusCheck.facter_timeout })
    { S0041: result.include?('java') }
  rescue Facter::Core::Execution::ExecutionFailure => e
    Facter.warn('puppet_status_check.S0041 failed to get socket status from SS')
    Facter.debug(e)
    { S0041: false }
  end
